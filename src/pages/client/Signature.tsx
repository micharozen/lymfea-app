    ...
    return (
      <div className="lymfea-client min-h-screen bg-[#FBF7F2] py-20 px-4 flex justify-center items-center text-[#2C2622]">
        <div className="w-full max-w-lg bg-white shadow-sm border border-[#E8DFD2] rounded-sm p-12 text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
          <h1 className="text-2xl font-serif tracking-widest uppercase mb-4 text-[#2C2622]">Document indisponible</h1>
          <div className="w-12 h-[1px] bg-[#C96A43] mx-auto mb-6"></div>
          <p className="text-[#8A7D6D] uppercase tracking-widest text-sm leading-relaxed">Ce lien est expiré ou le formulaire a déjà été complété et signé.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="lymfea-client min-h-screen bg-[#FBF7F2] py-20 px-4 flex justify-center items-center text-[#2C2622]">
        <div className="w-full max-w-lg bg-white shadow-sm border border-[#E8DFD2] rounded-sm p-12 sm:p-16 text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
          <h2 className="text-xs tracking-[0.3em] text-[#A8542F] uppercase mb-6">{bookingInfo?.hotel_name} by Eïa</h2>
          <h1 className="text-2xl sm:text-3xl font-serif tracking-widest uppercase mb-6 text-[#2C2622]">Votre expérience <br/> commence</h1>
    ...
    <div className="lymfea-client min-h-screen bg-[#FBF7F2] py-10 px-4 sm:px-8 flex flex-col items-center text-[#2C2622] selection:bg-[#C96A43] selection:text-white">
      ...
      <div className="w-full max-w-4xl bg-white shadow-sm border border-[#E8DFD2] rounded-sm p-6 sm:p-12 md:p-16 flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-sm tracking-[0.2em] text-[#8A7D6D] uppercase">Formulaire de Consentement</h2>
        ...
        <div className="flex flex-col gap-6 mb-16">
          <div className="flex flex-col md:flex-row gap-6 md:gap-8">
            <div className="flex items-baseline gap-3 flex-1 overflow-hidden">
              <span className="text-xs text-[#8A7D6D] uppercase tracking-widest shrink-0">Nom, Prénom :</span>
            ...
            <div className="flex items-baseline gap-3 md:w-1/3 shrink-0">
              <span className="text-xs text-[#8A7D6D] uppercase tracking-widest shrink-0">Date :</span>
          ...
          <div className="flex items-baseline gap-3 w-full md:w-2/3">
            <span className="text-xs text-[#8A7D6D] uppercase tracking-widest shrink-0">N° Chambre <span className="text-[10px] text-[#A89B89] tracking-normal hidden sm:inline">(Si client de l'hôtel)</span> :</span>
        ...
        <div className="mb-14">
          <h3 className="font-serif text-2xl uppercase tracking-widest mb-1 text-[#2C2622]">Informations Importantes Avant Le Soin :</h3>
          <p className="text-[11px] text-[#8A7D6D] uppercase tracking-widest mb-10 leading-relaxed border-b border-[#EFE7DA] pb-6">Afin de vous proposer une expérience sécurisée, adaptée à vos besoins et à votre état du moment, merci de bien vouloir répondre aux questions suivantes :</p>
          ...
          <div className="space-y-8">
            {[
              ...
              <div key={q.field} className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-3">
                ...
                <div className="flex items-center gap-6 shrink-0 pt-2 lg:pt-0">
                  <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-[#4A4038]"><input type="radio" name={q.field} value="oui" checked={formData[q.field as keyof typeof formData] === 'oui'} onChange={(e) => handleInputChange(q.field, e.target.value)} className="w-4 h-4 border-[#D9CDBC] focus:ring-[#C96A43] accent-[#C96A43]" /> OUI</label>
                  <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-[#4A4038]"><input type="radio" name={q.field} value="non" checked={formData[q.field as keyof typeof formData] === 'non'} onChange={(e) => handleInputChange(q.field, e.target.value)} className="w-4 h-4 border-[#D9CDBC] focus:ring-[#C96A43] accent-[#C96A43]" /> NON</label>
                </div>
              </div>
            ))}
            ...
            {[
              ...
              <div key={q.field} className="space-y-4">
                ...
                <div className="flex items-baseline gap-3 pl-0 lg:pl-4 pt-1">
                  <span className="text-xs text-[#8A7D6D] uppercase tracking-widest shrink-0">Si oui, précisez :</span>
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-3 mt-8 pt-8 border-t border-[#EFE7DA]">
              <label className="text-sm text-[#4A4038] uppercase tracking-widest leading-relaxed lg:pr-4">Avez-vous eu une intervention chirurgicale dans les 6 derniers mois :</label>
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 shrink-0 pt-2 lg:pt-0">
                <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-[#4A4038]"><input type="radio" name="recent_surgery" value="oui_accord" checked={formData.recent_surgery === 'oui_accord'} onChange={(e) => handleInputChange('recent_surgery', e.target.value)} className="w-4 h-4 border-[#D9CDBC] focus:ring-[#C96A43] accent-[#C96A43]" /> OUI, AVEC ACCORD MÉDICAL</label>
                <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-[#4A4038]"><input type="radio" name="recent_surgery" value="non" checked={formData.recent_surgery === 'non'} onChange={(e) => handleInputChange('recent_surgery', e.target.value)} className="w-4 h-4 border-[#D9CDBC] focus:ring-[#C96A43] accent-[#C96A43]" /> NON</label>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-14 pt-8 border-t border-[#EFE7DA]">
          <h3 className="font-serif text-2xl uppercase tracking-widest mb-10 text-[#2C2622]">Comment vous sentez-vous en ce moment ? :</h3>
          ...
          <div className="flex flex-col lg:flex-row gap-3 lg:items-baseline mt-10">
            <span className="shrink-0 text-sm text-[#4A4038] uppercase tracking-widest">Tensions localisées <span className="text-[10px] text-[#8A7D6D] tracking-normal">(ex. visage, mâchoires, trapèzes...)</span> :</span>
        ...
        <div className="mb-14 pt-8 border-t border-[#EFE7DA]">
          <h3 className="font-serif text-2xl uppercase tracking-widest mb-10 text-[#2C2622]">Intensité du massage souhaitée :</h3>

          <div className="flex flex-col lg:flex-row lg:items-center gap-6 mb-10">
            <span className="text-sm text-[#4A4038] uppercase tracking-widest shrink-0">Quelle intensité préférez-vous ?</span>
            <div className="flex flex-wrap items-center gap-6 sm:gap-8">
              {['leger', 'moyen', 'appuye'].map((intensity) => (
                <label key={intensity} className="flex items-center gap-2 cursor-pointer text-sm font-medium text-[#4A4038] uppercase tracking-wider">
                  <input type="radio" name="preferred_intensity" value={intensity} checked={formData.preferred_intensity === intensity} onChange={(e) => handleInputChange('preferred_intensity', e.target.value)} className="w-4 h-4 border-[#D9CDBC] focus:ring-[#C96A43] accent-[#C96A43]" />
          ...
          <div className="space-y-10">
            <div className="flex flex-col gap-3">
              <span className="text-sm text-[#4A4038] uppercase tracking-widest">Quelles sont les zones sur lesquelles vous souhaitez que l'on porte une attention particulière ?</span>
            ...
            <div className="flex flex-col gap-3">
              <span className="text-sm text-[#4A4038] uppercase tracking-widest">Y a-t-il des zones que vous préférez que l'on évite ?</span>
        ...
        <div className="mb-14 bg-[#F8F3EC] p-6 sm:p-10 border border-[#E8DFD2] rounded-sm">
          <h3 className="font-serif text-xl uppercase tracking-widest mb-8 text-[#2C2622]">
            Clause de facturation <span className="text-xs text-[#8A7D6D] tracking-normal ml-0 sm:ml-2 block sm:inline mt-2 sm:mt-0">(Clients de l'hôtel uniquement) :</span>
          </h3>
          ...
          <div className="text-sm text-[#4A4038] leading-loose uppercase tracking-wide">
            ...
            <div className="mt-8 flex flex-col md:flex-row gap-6 md:gap-8 md:items-baseline">
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-3 flex-1">
                <span className="shrink-0 text-[#8A7D6D] text-xs tracking-widest">TYPE DE PRESTATION :</span>
                <div className="flex-1 border-b border-[#D9CDBC] pb-1 text-[#2C2622] font-serif text-lg">{bookingInfo?.treatment_name}</div>
              </div>
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-3 w-full md:w-1/3">
                <span className="shrink-0 text-[#8A7D6D] text-xs tracking-widest">MONTANT :</span>
                <div className="flex-1 border-b border-[#D9CDBC] pb-1 text-[#2C2622] font-serif text-lg sm:text-right pr-2">{bookingInfo?.price}</div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-10 flex items-center gap-4">
            ...
            <label htmlFor="confirm-billing" className="text-sm font-semibold uppercase tracking-widest cursor-pointer text-[#2C2622]">Je confirme</label>
          </div>
        </div>

        {/* SECTION 5 : CONSENTEMENT & SIGNATURE */}
        <div className="mb-4">
          <h3 className="font-serif text-2xl uppercase tracking-widest mb-6 text-[#2C2622]">Consentement Éclairé :</h3>
          <div className="text-[11px] text-[#6B6055] space-y-4 uppercase tracking-widest leading-relaxed text-justify mb-10">
            <p>Le client déclare que les données concernant ses informations personnelles, expressément demandées dans le but d'évaluer la compatibilité avec les soins fournis, sont vraies. Le client dégage donc {bookingInfo?.hotel_name} et Eïa de toute responsabilité quant à tout problème découlant de l'omission intentionnelle ou non d'informations concernant tout état pathologique, allergique, inflammatoire ou autre, qui peut compromettre l'état de santé de ce dernier. {bookingInfo?.hotel_name} et Eïa déclarent respecter les règles de confidentialité du client sur la base de la législation en vigueur et ne pas divulguer à des tiers les données à caractère personnel de ce dernier, que ce soit à des fins lucratives ou commerciales.</p>
          </div>

          <div className="mt-10">
            <p className="text-sm font-bold uppercase tracking-widest mb-6 text-center text-[#2C2622]">Signature du client : <br/><span className="text-[10px] font-normal text-[#8A7D6D] tracking-widest mt-1 block">Précédée de la mention « Lu et approuvé »</span></p>
            <div className="border border-[#E8DFD2] p-2 bg-[#F8F3EC] relative shadow-inner rounded-sm">
              <span className="absolute top-4 left-6 text-[#D9CDBC] font-serif italic pointer-events-none select-none text-lg">Lu et approuvé...</span>
              <canvas
