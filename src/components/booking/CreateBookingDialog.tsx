...
export default function CreateBookingDialog({ open, onOpenChange, selectedDate, selectedTime, presetHotelId }: CreateBookingDialogProps) {
  ...
  // Reset additional therapists when required count drops back to 1
  useEffect(() => {
    if (requiredGuestCount <= 1) setAdditionalTherapistIds([]);
  }, [requiredGuestCount]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mutation.mutate({
      ...
      ...(requiredGuestCount > 1 && additionalTherapistIds.length > 0
