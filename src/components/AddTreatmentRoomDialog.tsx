...
export function AddTreatmentRoomDialog({
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (error) {
      toast.error(t('admin:treatmentRoomDialog.addError'));
      return;
    }

    toast.success(t('admin:treatmentRoomDialog.added'));
    form.reset();
    setRoomImage("");
    onOpenChange(false);
    onSuccess();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{t('admin:treatmentRoomDialog.addTitle')}</DialogTitle>
        </DialogHeader>
