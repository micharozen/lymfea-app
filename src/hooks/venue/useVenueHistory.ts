...
async function resolveUserNames(userIds: string[]): Promise<Record<string, string>> {
  ...
  for (const a of admins ?? []) {
    if (a.user_id) names[a.user_id] = `${a.first_name} ${a.last_name}`.trim();
  }
  ...
  if (remaining.length > 0) {
    ...
    for (const c of concierges ?? []) {
      if (c.user_id) names[c.user_id] = `${c.first_name} ${c.last_name}`.trim();
    }
  }
  ...
  if (remaining2.length > 0) {
    ...
    for (const t of therapists ?? []) {
      if (t.user_id) names[t.user_id] = `${t.first_name} ${t.last_name}`.trim();
    }
  }
...
export function useVenueHistory(hotelId: string | undefined, enabled: boolean) {
  return useQuery<VenueAuditEntry[]>({
    queryKey: ["venue-history", hotelId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("audit_log")
        .select("id, changed_at, changed_by, change_type, old_values, new_values, source, metadata")
        .eq("table_name", "hotels")
        .eq("record_id", hotelId!)
        .order("changed_at", { ascending: false });

      if (error) throw error;

      const entries = (data ?? []) as unknown as Omit<VenueAuditEntry, "changed_by_name">[];
