...
<Note>
  This page describes how Saoma models bookings. Creating and managing bookings
  **through the Partner API** (`POST /v1/bookings`, …) is **coming soon** —
  see the API Reference pages tagged _Coming soon_.
</Note>
...
| `cancelled`             | Cancelled by customer, partner, venue or Saoma                          |
| `noshow`                | Customer did not show up for the appointment                          |

<Note>
  In most cases, bookings created via `POST /v1/bookings` land directly in
  `confirmed` because the Saoma engine assigns a therapist atomically at
...
<ResponseField name="confirmation_url" type="string">
  Link to the customer-facing confirmation page on Saoma.
</ResponseField>
