...
serve(async (req) => {
  ...
  try {
    ...
    // Check for DEV_MODE to bypass Twilio verification
    const DEV_MODE = Deno.env.get('DEV_MODE') === 'true';
    
    if (DEV_MODE && code === '123456') {
      console.log('🔧 DEV MODE: Bypassing Twilio verification');
      console.log('✅ OTP verified successfully (DEV MODE)');
      // Skip to session creation
    } else if (DEV_MODE) {
      console.log('⚠️ DEV MODE: Invalid dev code');
      return new Response(
        JSON.stringify({ 
          success: false,
          error: 'Code incorrect (utilisez 123456 en mode dev)' 
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    } else {
