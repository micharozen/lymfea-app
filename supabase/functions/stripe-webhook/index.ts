  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (session.mode === "payment" && session.payment_status === "paid") {
        ...
        const { data: bookingTreatments } = await supabase
          .from('booking_treatments')
          .select('treatment_id, therapist_id, price_override, treatment_menus(name, price, price_on_request, duration), treatment_variants(price, duration)')
          .eq('booking_id', booking.id);
        ...
        const { data: booking, error: bookingError } = await supabase
          ...
          .select(`
            *,
            ...
            booking_treatments(therapist_id, treatment_menus(duration))
          `)
          .eq('id', bookingId)
          .single();
