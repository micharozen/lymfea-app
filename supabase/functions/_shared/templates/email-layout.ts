...
export function iconTile(svg: string): string {
  return `<td width="52" class="eia-tile" style="width:52px;vertical-align:top;padding:0 14px 0 0"><table role="presentation" border="0" cellpadding="0" cellspacing="0"><tbody><tr><td width="38" height="38" align="center" valign="middle" style="width:38px;height:38px;background-color:${SAND_50};border:1px solid ${LINE_SOFT};border-radius:10px;text-align:center;vertical-align:middle;line-height:38px;font-size:0">${svg}</td></tr></tbody></table></td>`;
}
...
export function detailsBox(copy: EmailCopy, therapistRow: string): string {
  ...
  return `<tr><td class="eia-sect" style="padding:28px 40px 0"><table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0" style="background-color:${SAND_100};border:1px solid ${LINE_SOFT};border-radius:14px"><tbody><tr><td class="eia-box" style="padding:22px 22px"><table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0"><tbody>${whenRow}${keyGap()}${whereRow}${therapistRow}</tbody></table></td></tr></tbody></table></td></tr>`;
}

/** Treatments section (line per treatment) + clay-tint total block. */
export function treatmentsSection(copy: EmailCopy): string {
  return `<tr><td class="eia-sect" style="padding:28px 40px 0"><p class="eia-tsect-title" style="margin:0 0 10px;font-family:${FONT_MONO};font-size:10px;letter-spacing:0.1em;text-transform:uppercase;color:${INK_MUTE}">{{{section_title}}}</p><table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0"><tbody>{{{treatments_html}}}</tbody></table><table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0" class="eia-total-table" style="margin-top:16px;background-color:${CLAY_TINT};border-radius:12px"><tbody><tr><td style="padding:16px 20px;font-family:${FONT_MONO};font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:${CLAY}">${copy.totalLabel}</td><td align="right" style="padding:16px 20px;font-family:${FONT_SERIF};font-size:24px;color:${INK}">{{{total_price}}}</td></tr></tbody></table></td></tr>`;
}
