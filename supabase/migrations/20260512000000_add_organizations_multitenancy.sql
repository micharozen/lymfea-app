...
-- Tables scoped via booking_id (overlay batch). Each entry is gated for
-- admin role only — non-admin roles keep their existing policies intact.
DO $$
DECLARE
  tbl text;
  tables_with_booking_id text[] := ARRAY[
...
BEGIN
  FOREACH tbl IN ARRAY tables_with_booking_id LOOP
    IF EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = tbl AND column_name = 'booking_id'
    ) THEN
      EXECUTE format('DROP POLICY IF EXISTS %I ON public.%I', 'Admin org isolation', tbl);
      EXECUTE format(
        'CREATE POLICY %I ON public.%I '
        'AS RESTRICTIVE TO authenticated '
        'USING (public.admin_can_access_booking(booking_id)) '
...
-- -------------------------------------------------------------------------
-- 14. BEFORE INSERT trigger on hotels: auto-fill organization_id
--     This is the *centralization point* for org assignment on hotels — the
--     application code does NOT need to pass `organization_id` on insert.
--     Resolution order:
--       1. If the caller already provided organization_id, keep it (but RLS
--          will still verify the value via the WITH CHECK on hotels INSERT).
--       2. Otherwise, fill from get_user_organization_id(auth.uid()) — i.e.
--          the calling admin's organization.
--       3. Fallback to the Lymfea Default org (so service_role inserts and
--          super-admins acting without a chosen scope still work).
-- -------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION public.set_hotel_organization_id_default()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DROP TRIGGER IF EXISTS hotels_default_organization_id ON public.hotels;
CREATE TRIGGER hotels_default_organization_id
  BEFORE INSERT ON public.hotels
  FOR EACH ROW
  EXECUTE FUNCTION public.set_hotel_organization_id_default();

COMMENT ON FUNCTION public.set_hotel_organization_id_default() IS
  'Auto-fills hotels.organization_id from the current admins.organization_id so application code never needs to pass it explicitly.';
