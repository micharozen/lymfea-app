-- =========================================================================
-- Add `availability:read` scope to org API keys
-- =========================================================================
-- The availability API (GET /v1/venues/:id/availability[/treatments/:id]) is
-- gated by a new `availability:read` scope. We patch the org key generator to
...
-- Mirrors 20260611140000_add_customers_read_scope.sql.
-- =========================================================================

-- 1. Patch gateway_create_org_api_key to include `availability:read`.
...
BEGIN
  ...
  VALUES (
    ...
    v_prefix,
    v_secret_id,
    ARRAY['venues:read', 'treatments:read', 'customers:read', 'availability:read', 'bookings:read', 'therapists:read'],
    NULL
  )
...
UPDATE gateway.api_keys
   SET scopes = array_append(scopes, 'availability:read')
 WHERE revoked_at IS NULL
   AND organization_id IS NOT NULL
   AND NOT ('availability:read' = ANY (scopes));
