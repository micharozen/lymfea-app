-- ============================================================
-- Exclusivité et délai de remise en état des commodités.
--
...
--    réservation : rien ne peut démarrer pendant le nettoyage. Le nettoyage
--    peut déborder après l'heure de fermeture, il n'ampute pas le dernier
--    créneau vendable.
--
-- La règle vit dans une seule fonction SQL, `amenity_slot_conflict`, appelée
-- par reserve_trunk_atomically (site client, bloquant) et exposée en RPC pour
-- l'admin, où elle sert d'avertissement contournable.
-- ============================================================
...
COMMENT ON COLUMN public.venue_amenities.capacity_per_slot IS
  ...
  'Sinon : cumulé sur le créneau, toutes réservations confondues.';

COMMENT ON COLUMN public.venue_amenities.prep_time IS
...
--   'AMENITY_EXCLUSIVE' → créneau déjà pris (nettoyage inclus) sur une
--                         commodité privatisable
--   'AMENITY_FULL'      → capacité insuffisante
-- `_exclude_amenity_booking_id` permet de réévaluer une ligne en cours d'édition
-- sans qu'elle se bloque elle-même.
CREATE OR REPLACE FUNCTION public.amenity_slot_conflict(
  _venue_amenity_id uuid,
  _booking_date date,
  _start_time time,
  _end_time time,
  _guests integer DEFAULT 1,
  _exclude_amenity_booking_id uuid DEFAULT NULL
) RETURNS text
...
ALTER FUNCTION public.amenity_slot_conflict(uuid, date, time, time, integer, uuid) OWNER TO postgres;

COMMENT ON FUNCTION public.amenity_slot_conflict(uuid, date, time, time, integer, uuid) IS
  'Règle unique de conflit sur une commodité : exclusivité, capacité et remise '
  'en état. NULL = réservable, sinon AMENITY_EXCLUSIVE ou AMENITY_FULL.';

GRANT ALL ON FUNCTION public.amenity_slot_conflict(uuid, date, time, time, integer, uuid) TO anon;
GRANT ALL ON FUNCTION public.amenity_slot_conflict(uuid, date, time, time, integer, uuid) TO authenticated;
GRANT ALL ON FUNCTION public.amenity_slot_conflict(uuid, date, time, time, integer, uuid) TO service_role;

-- 3. reserve_trunk_atomically délègue à la règle commune ----------------------
-- Seul le bloc de contrôle des commodités change : le calcul d'occupation en
-- ligne est remplacé par l'appel à amenity_slot_conflict, sous le même verrou.
