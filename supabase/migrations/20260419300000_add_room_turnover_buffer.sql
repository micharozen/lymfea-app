-- ==============================================================================
-- Migration : add_room_turnover_buffer
-- Description : Ajoute un buffer de remise en état des salles de soin.
--   Après chaque prestation, la salle ET le thérapeute sont bloqués pendant
--   room_turnover_buffer_minutes pour permettre le nettoyage/réorganisation.
-- ==============================================================================

-- 1. Nouveau champ sur hotels : buffer en minutes (0 = pas de buffer)
ALTER TABLE public.hotels
  ADD COLUMN IF NOT EXISTS room_turnover_buffer_minutes INTEGER DEFAULT 0;

-- 2. Recréation de reserve_trunk_atomically() avec buffer de remise en état
DO $$
DECLARE
    _drop_stmt text;
BEGIN
