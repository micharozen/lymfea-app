-- ============================================================
-- Jours de disponibilité par variante de soin.
--
-- Contexte : un lieu décline un même soin en variantes tarifaires « Semaine » /
-- « Week-end » (prix différents selon le jour). La contrainte de jours n'existait
-- qu'au niveau du soin (treatment_menus.available_days), donc l'écran de créneaux
-- proposait le week-end alors que la variante « Semaine » était sélectionnée.
--
-- 1. treatment_variants.available_days (0=Dim … 6=Sam ; NULL/vide = hérite du soin)
-- 2. get_public_treatments : expose available_days dans le jsonb des variantes
-- 3. reserve_trunk_atomically : nouveau paramètre _variant_ids + contrôle
--    DAY_CONSTRAINT_VIOLATION jumeau de celui des soins
-- ============================================================

-- 1. Colonne ----------------------------------------------------------------
ALTER TABLE public.treatment_variants
  ADD COLUMN IF NOT EXISTS available_days integer[];

COMMENT ON COLUMN public.treatment_variants.available_days IS
  'Jours autorisés : 0=Dim, 1=Lun, ..., 6=Sam. NULL/vide = hérite des jours du soin parent. '
  'Permet des variantes tarifaires Semaine / Week-end.';

-- 2. RPC publique : exposer available_days par variante ----------------------
...
-- 3. reserve_trunk_atomically : contrainte de jours au niveau de la variante --
--
-- Ajouter un paramètre change la signature : CREATE OR REPLACE créerait une
-- SECONDE fonction, et PostgREST route selon les clés envoyées par l'appelant
-- (cf. 20260720150000_drop_stale_reserve_trunk_overload.sql). On drop donc toute
-- surcharge dépourvue de _variant_ids avant de recréer la fonction.
DO $drop$
DECLARE _s text;
BEGIN
  FOR _s IN (
    ...
    WHERE proname = 'reserve_trunk_atomically'
      ...
      AND NOT ('_variant_ids' = ANY (COALESCE(proargnames, ARRAY[]::text[])))
  ) LOOP
...
-- Le DROP ci-dessus a emporté les privilèges : le flow client est public (anon),
-- la fonction doit rester appelable sans auth.
...
-- Garde-fou : une seule définition doit subsister.
DO $check$
DECLARE _n integer;
BEGIN
  ...
  IF _n <> 1 THEN
    RAISE EXCEPTION 'reserve_trunk_atomically : % définitions au lieu de 1', _n;
  END IF;
END $check$;
