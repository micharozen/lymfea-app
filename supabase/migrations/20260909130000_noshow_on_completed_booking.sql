-- Un soin du matin est passé 'completed' (cron complete-past-paid-bookings, ou
-- clôture manuelle) avant que l'équipe n'ait pu signaler que le client n'était
-- pas venu : le bouton « Client pas venu » devenait inopérant. On autorise donc
-- le passage completed → noshow ; seules les réservations annulées ou déjà en
-- no-show restent figées.
--
-- Reprise à l'identique de begin_booking_noshow (20260520113000), garde du
-- statut mise à jour.
...
BEGIN
  ...
  IF _current_status IN ('cancelled', 'noshow') THEN
