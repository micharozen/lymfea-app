-- Restore booking_hold_enabled and booking_hold_duration_minutes in
-- get_public_hotel_by_id. They were added in 20260421000000 but dropped by
-- 20260423120000 which recreated the RPC to add address fields.
--
-- Without these, the client-side SchedulePanel falls back to holdEnabled=true
-- for venues where hold is disabled, then calls create-draft-booking which
-- returns { success: false, reason: 'hold_disabled' }.
