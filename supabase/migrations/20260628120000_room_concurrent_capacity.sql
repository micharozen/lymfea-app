-- Salles de soin à plusieurs lits : la capacité d'une salle (treatment_rooms.capacity)
-- représente le nombre d'occupations simultanées (lits). reserve_trunk_atomically
-- comptait jusqu'ici une salle comme entièrement bloquée dès la première réservation.
-- On remplace ce blocage binaire par une somme des `guest_count` qui chevauchent,
-- comparée à la capacité de la salle (un duo = 2 lits).
