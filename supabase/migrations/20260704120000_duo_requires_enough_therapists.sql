-- Un duo (guest_count > 1) exige autant de thérapeutes qualifiés + disponibles que
-- de personnes traitées simultanément. reserve_trunk_atomically ne validait qu'UN
-- seul thérapeute par salle : un créneau où un seul praticien est libre pouvait être
-- réservé en duo puis rester impossible à honorer. On remplace le « trouve-en un »
-- par un compteur : la salle n'est retenue que si >= guest_count thérapeutes
-- distincts, qualifiés et sans conflit, sont disponibles sur le créneau.
-- Aligne l'écriture atomique sur ce que get-availability promet déjà côté affichage
-- (capacity = min(lits libres, thérapeutes libres) >= guest_count).
...
BEGIN
  ...
  LOOP
    ...
    -- Count qualified + available therapists (gender-aware for solo). A duo needs
    -- at least `guest_count` distinct practitioners free on this slot, not just one.
    ...
    LOOP
      ...
      IF NOT _has_conflict THEN
        ...
        -- Room is valid once enough distinct practitioners are free (1 for solo,
        -- guest_count for a duo). `_therapist_id` holds the last counted one and
        -- is only stored for solo bookings below (duo → NULL, assigned later).
        IF _qualified_available >= COALESCE(_guest_count, 1) THEN
          _room_id := _room.id;
          EXIT room_loop;
        END IF;
      END IF;
